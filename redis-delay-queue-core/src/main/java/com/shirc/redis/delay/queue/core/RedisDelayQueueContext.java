...
import com.shirc.redis.delay.queue.iface.impl.RedisDelayQueueImpl;
import com.shirc.redis.delay.queue.redis.RedisOperation;
import com.shirc.redis.delay.queue.redis.RedisOperationByLua;
...
public class RedisDelayQueueContext {
    ...
    public RedisDelayQueueContext(RedisTemplate<Object, Object> redisTemplate, String projectName) {
        initCtx(redisTemplate, projectName, null);
    }

    public RedisDelayQueueContext(RedisTemplate<Object, Object> redisTemplate, String projectName, String ipInRedisServer) {
        initCtx(redisTemplate, projectName, ipInRedisServer);
    }

    protected final void initCtx(RedisTemplate<Object, Object> redisTemplate, String projectName, String ipInRedisServer) {
        if (projectName.contains(RedisKeyUtil.SINGLE_COLON)
                || projectName.contains(RedisKeyUtil.LEFT_BRACKET)
                || projectName.contains(RedisKeyUtil.RIGHT_BRACKET)) {
    ...
    private void runTopicThreads(AbstractTopicRegister register) {
                        ...
                        for (int i = 0; i < topicIds.size(); i++) {
                            ...
                            register.getTOPIC_THREADS().execute(() -> {
                                ...
                                try {
                                    ...
                                    if (args != null) {
                                        try {
                                            this.checkTimeoutExecute(register.getMethodTimeout(), register, args);
                                    ...
                                    if (args != null && (!isfail || (args.getRetryCount() > 2 || args.getRetryCount() < 0))) {
        ...
        a.setDaemon(true);
        a.setName("监听" + register.getTopic());
        a.start();
    }
     ...
     */
    private void checkTimeoutExecute(final long timeout, AbstractTopicRegister register, Args args) throws InterruptedException, ExecutionException, TimeoutException {
    ...
    public static void addTopic(String topic, AbstractTopicRegister register) {
    ...
    private static void setTopicThreadStop(boolean topicThreadStop) {
    ...
    private void killThisMachineAllRedisBlpopClients(RedisOperation redisOperation) {
        ...
        for (RedisClientInfo info : list) {
            if (info.getAddressPort().split(RedisKeyUtil.SINGLE_COLON)[0].equals(ip)
